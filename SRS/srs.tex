...
The generated result will consist of studied and selected applications where a set of instructions, going from a single instruction up to an entire section
...
This research on ASIPs is an attempt to expand on the computing paradigm, which is expected to 
enable more personalized applications that adjust flexibility and performance to achieve an
optimal balance between the two variables.

The ASIPs that will be integrated to a specific application will have the following characteristics:

\begin{itemize}
 \item Adjustable error threshold: The application's error level can vary according to a specific ASIP configuration.
 
 \item Adjustable resource consumption: The application's resource consumption can vary according to a specific ASIP configuration.
...
The following limitations are considered for the developed ASIPs:
\begin{itemize}
 ...
 be developed in that amount of time.
\end{itemize}
...
\begin{tabular}{|c|c|} 
 ...
 they are not optimized, they show less resource efficiency.}  \\ \hline
   ASIC & \makecell{
 Application Specific Integrated Circuits. In general, they show better performance \\
...
As explained in section \ref{sec:user}, the users are expected to be researchers, for this, only a few 
...
\begin{itemize}
 ...
 \item Development framework: The ASIPs are going to be developed with the use of ASIPMeister and Dlxsim,
 which are already present on the work environment.
...
Approximate computing has been studied by several authors, the survey \cite{xu2018approximate} gathers several ways to implement
this paradigm both by hardware or by software. 	
