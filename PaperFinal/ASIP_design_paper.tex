...
\emph{NOTA: el abstract lo terminamos de definir/pulir una vez que el paper este listo.}
\end{abstract}
...
approximate versions, so that a balance can be obtained between the output quality
 and the general computing resources required.

\emph{NOTA: Podrias mencionar los diferentes niveles de abstraccion a los cuales se ha 
...
\emph{NOTA: Una vez que el resto de las secciones estén completas, hay que iterarar 
...
\emph{NOTA: el contenido de este párrafo se puede usar para la introducción y no acá. La 
