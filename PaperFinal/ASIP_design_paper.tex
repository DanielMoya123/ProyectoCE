...
ALU is ignored and instead the result from the special additional hardware is used. 
...
\node[rectangle, rounded corners,draw=black, very thick, minimum size=3em, above = 1cm of alu] (asip) {\makecell{ASIP\\Logic}};

\draw (alu.220) -- ++(180:3mm) -- ++(90:41mm)  -|  (asip.191);
\draw (alu.140) -- ++(180:8mm) -- ++(90:28mm)  -|  (asip.153);

\draw (asip.east) -- ++(0:15mm) node [right] (asipResult) {\makecell{ASIP\\Result}};
